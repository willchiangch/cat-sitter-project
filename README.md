...
**目前版本：V31 (訂閱折扣碼系統 + 免費啟用方案 + 方案變更歷程 log)**
...
| 資料庫版控| Flyway（Schema V27: 訂閱折扣碼 + 方案變更歷程）|
| E2E 狀態 | **Playwright 57/57 全數通過**（含 V31 折扣碼訂閱 + V30 認證標章 + 訂閱合約協議驗證） |
...
- **訂閱方案管理（折扣碼 + 合約協議）**：保母可在 `/sitter/subscription` 切換方案；Agreement modal 提供折扣碼輸入（即時驗證、顯示原價/折扣/實付），折扣後歸零直接免費啟用（不走付款流程）；後端以 `subscription_change_logs` 全量記錄每次方案變更。
- **自動化 Onboarding**：全新社交登入使用者自動偵測並強制導航至身分設定流程。
- **身分驗證標章 (Blue Checkmark)**：保母 Profile 與 Dashboard 名字旁新增認證標章；支援「未認證 / 審核中 / 已認證」三段式狀態視覺回饋。
- **財務與訂閱**：整合 PAYUNi 金流，支援保母訂閱方案與促銷折扣碼；Finance 頁分為「待付款」與「收款紀錄」雙 tab；銀行資訊（代碼/帳號）支援原子性更新。
...
```bash
cd frontend
npx playwright test    # 執行所有 POM 化後的 E2E 腳本（目前 57/57 通過）
```

> **E2E 測試覆蓋範圍（V25 + V26 + V30 + V31）**：
...
> - V30 功能驗收（保母認證標章三段式狀態、訂閱合約協議彈窗四條款、關閉彈窗流程）
> - V31 功能驗收（折扣碼 UI 存在性、有效碼折扣顯示、無效碼錯誤訊息、折扣歸零免費啟用按鈕、Enter 鍵觸發驗證）
