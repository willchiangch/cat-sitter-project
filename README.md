...
**目前版本：V30 (保母身分驗證標章 + 訂閱合約協議流程 + 銀行資訊儲存修復 + 導航高亮修正 + E2E 驗證)**
...
| E2E 狀態 | **Playwright 46/46 全數通過**（含 V30 認證標章 + 訂閱合約協議驗證） |
...
- **訂閱方案管理（含合約協議）**：保母可在 `/sitter/subscription` 內切換方案，新增「服務合約協議」流程，包含自動續約、扣款失敗降級、不支援退費以及升級補差價規則說明。
...
```bash
cd frontend
npx playwright test    # 執行所有 POM 化後的 E2E 腳本（目前 46/46 通過）
```

> **E2E 測試覆蓋範圍（V25 + V26 + V30）**：
