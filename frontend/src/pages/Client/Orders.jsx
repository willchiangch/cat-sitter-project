        ...
        <AnimatePresence mode="popLayout">
          {filteredOrders.length > 0 ? (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                       <div className="flex justify-between items-start">
                          <h4 className="text-sm font-extrabold tracking-tight">{order.sitterName}</h4>
                          <span className="text-[10px] font-bold text-primary">${order.amount}</span>
                       </div>
                       <p className="text-[10px] font-bold opacity-30 mt-1 uppercase tracking-widest">{order.catNames}</p>
                       <p className="text-[10px] font-bold opacity-30 mt-0.5">{order.date}</p>
                    </div>
                  </div>

                  <div className="flex gap-2">
                    {order.orderStatus === 'QUOTED' && (
                       <button className="flex-1 py-3 bg-primary text-on-primary rounded-full text-[10px] font-extrabold uppercase shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all">
                          前往付款 (憑證上傳)
                       </button>
                    )}
                    {order.orderStatus === 'CONFIRMED' && (
                       <button 
                         ...
                         className="flex-1 py-3 bg-primary text-on-primary rounded-full text-[10px] font-extrabold uppercase shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all"
                       >
                          查看即時日誌
                       </button>
                    )}
                    <button 
                      ...
                      className="flex-1 py-3 bg-surface-container-high text-on-surface text-[10px] font-extrabold uppercase rounded-full hover:bg-surface-container-highest transition-colors"
                    >
                       訂單詳情
                    </button>
                  </div>
            ...
            >
              ...
              <p className="text-xs font-bold tracking-widest uppercase">目前尚無此類別的預約</p>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.main>
