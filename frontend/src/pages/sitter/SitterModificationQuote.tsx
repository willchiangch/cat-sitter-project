  ...
  return (
    <div style={{ padding: '2rem 1.5rem', fontFamily: 'var(--font-sans)', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <Card>
          <h3 style={{ margin: '0 0 1.5rem 0', fontSize: '1.25rem', fontFamily: 'var(--font-display)' }}>變更報價調整</h3>
          {quoteSent ? (
            <div style={{ textAlign: 'center', padding: '1rem', color: 'var(--color-primary)', fontWeight: '600' }} data-testid="quote-success-banner">
              ✓ 報價已送出 (總額: $ {adjustedAmount.toLocaleString()})
            </div>
          ) : (
            <form onSubmit={handleSendQuote} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <Card>
          <h3 style={{ margin: '0 0 1.5rem 0', fontSize: '1.25rem', fontFamily: 'var(--font-display)' }}>退款轉帳憑證上傳 (若涉及退款)</h3>
          {proofSent ? (
            <div style={{ textAlign: 'center', padding: '1rem', color: 'var(--color-primary)', fontWeight: '600' }} data-testid="refund-proof-success-banner">
              ✓ 退款憑證已成功上傳
            </div>
          ) : (
            <form onSubmit={handleUploadProof} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label style={{ fontSize: '0.875rem', fontWeight: '600' }}>憑證 / 收據圖片網址 (GCP Storage)</label>
                <input 
                  type="text" 
