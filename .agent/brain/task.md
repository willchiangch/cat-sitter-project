...
- [x] **CI 修復：`PaymentControllerTest` `order_logs` 外鍵違反**（2026-07-24）：push 上述批次後 GitHub Actions「Build and Deploy to GCP」的 `Run Backend Tests` 步驟失敗（`order_logs_order_id_fkey` 違反）。根因是 `PaymentControllerTest.setUp()` 只清 `orders` 沒先清 `order_logs`，姊妹測試類別（`CompletionServiceTest`/`VisitReportControllerTest`）早有此慣例，唯獨這支一直沒補，疑似因新增 `WebAuthnControllerTest` 改變 CI 全套件測試類別執行順序而首次觸發，與本輪功能程式碼無關，屬既有測試孤立性 debt。補上 `orderLogRepository.deleteAll()` 後本地 207/207 綠燈，推送後 CI 重新轉綠（詳見 walkthrough.md 第 14.5 節）(✅ **Implemented**)
