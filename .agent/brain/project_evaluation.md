...
### 4. 前端 UI/UX 嚴重脫節 (Critical Frontend Discrepancy)
- **現狀**：在最近一次人工巡檢中發現，目前前端介面僅為空殼，且各 Tab 的名稱、元件放置位置、以及具備的核心功能（如方案設定、信任圈、客群名單門禁、寵物管理等），與 `doc/frontend-spec.md` 及 `doc/business-requirements.md` 發生嚴重脫節。此外，全域的主題配色缺乏高對比遮罩，導致部分內容難以辨識。
- **調整建議**：必須立刻啟動「前端全面重構計畫」，嚴格按照前端需求規格書，校正所有組建的相對位置與資料流，以避免功能持續開發在錯誤的基礎上。
